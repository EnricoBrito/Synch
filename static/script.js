...
form.addEventListener("submit", (e) => {
  ...
  const subject = document.getElementById("email").value.trim(); 
  const message = document.getElementById("message").value.trim();
  
  if (!name || !subject || !message) return;

  const mailtoLink = `mailto:synchsuporte@gmail.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent("Nome: " + name + "\n\n" + message)}`;
